  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <div>
          <Link to="/templates" className="mr-2 border rounded-md px-4 py-2 text-sm">
            템플릿 보기
          </Link>
          <Link to="/routines/new"
            className="flex items-center gap-2 bg-indigo-600 text-white font-semibold px-4 py-2 rounded-md hover:bg-indigo-700"
          >
              ...
              <div>
                ...
                <ul className="mt-2 text-gray-600 list-decimal list-inside">
                  {routine.details.map((detail) => (
                    <li key={detail.id}>
                      {detail.name} ({detail.set}세트, {detail.rep}회)
                    </li>
                  ))}
                </ul>
              </div>
